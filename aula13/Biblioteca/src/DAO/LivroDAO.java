...
public class LivroDAO {
   ...
   public static void editarLivro(int idLivro, String novoNome,String novoAutor,String novoGenero,String novoISBN){
       ...
       String sql="UPDATE livro SET nome_livro=?, autor=?,genero=?,isbn=? WHERE id_livro=?";
       Connection conexao=null;
       PreparedStatement stmt=null;
    ...
    public static void editarLivro(Livro LivroEditado){
        ...
        try{conexao=conectar();
            if(conexao!=null){
                stmt=conexao.prepareStatement(sql);
                stmt.setString(1,LivroEditado.getNome());
                stmt.setString(2, LivroEditado.getAutor());
                stmt.setString(3,LivroEditado.getGenero());
                stmt.setString(4,LivroEditado.getGenero());
                stmt.setInt(5,LivroEditado.getId());
                int linhasAfetadas=stmt.executeUpdate();
                if(linhasAfetadas>0){
                    System.out.println("Livro com ID "+LivroEditado.getId()+" atualizado com sucesso!");
                }else{
                    System.out.println("Nenhum aluno encontrado com ID "+LivroEditado.getId()+" para atualização.");
                }
            }
        }
        catch(SQLException e){
